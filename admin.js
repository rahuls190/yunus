...
function editArtwork(index) {
  ...
  document.getElementById('artworkCategory').value = art.category || 'all';
  document.getElementById('artworkSubcategory').value = art.subcategory || 'all';
...
async function submitArtworkForm() {
  ...
  const subcategoryInput = document.getElementById('artworkSubcategory');
...
async function updateArtwork(index, src, title, medium, description, category, subcategory, featured) {
  const art = currentArtworks[index];
...
async function saveQuote() {
  ...
  if (window.supabase) {
    try {
      ...
      toast('Failed to save quote.', true);
    }
  } else {
...
async function loadBlogAdmin() {
  currentBlogPosts = await fetchBlogPosts();
  const tableBody = document.getElementById('blogTableBody');
  if (!tableBody) return;
  tableBody.innerHTML = '';
  
  if (currentBlogPosts.length === 0) {
    tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding: 20px;">No blog posts found.</td></tr>';
    return;
  }
  ...
  currentBlogPosts.forEach(post => {
    ...
    const imgHtml = post.image_url 
      ? `<img src="${post.image_url}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px;">`
    ...
    tr.innerHTML = `
      ...
      <td>
        <span class="status-badge ${post.published ? 'status-active' : 'status-inactive'}">
          ${post.published ? 'Published' : 'Draft'}
        </span>
      </td>
      <td>
        <button class="btn btn-secondary" onclick="editBlog(${post.id})" style="padding: 4px 8px; font-size: 0.75rem;">Edit</button>
        <button class="btn btn-secondary" onclick="deleteBlog(${post.id})" style="padding: 4px 8px; font-size: 0.75rem; color: #d32f2f;">Delete</button>
      </td>
    `;
...
function editBlog(id) {
  ...
  document.getElementById('blogImagePath').value = post.image_url || '';
...
async function saveBlog() {
  if (!window.supabase) {
    toast('Database not connected.', true);
    return;
  }
  ...
  let imagePath = document.getElementById('blogImagePath').value.trim();
  
  if (!title || !content) {
    toast('Title and Content are required.', true);
    return;
  }
  ...
  try {
    if (editingBlogId) {
      const { error } = await supabaseClient.from('blog_posts').update(payload).eq('id', editingBlogId);
      if (error) throw error;
    ...
    toast('Error saving blog: ' + err.message, true);
  }
}
...
async function deleteBlog(id) {
  ...
  try {
    ...
    toast('Error deleting blog: ' + err.message, true);
  }
}
